...
proportion_results %>%
  map(function(x){
    tibble(Grassland = pct_count(x = x, r = 1), 
           Shrubland = pct_count(x = x, r = 2),
           Conifer = pct_count(x = x, r = 3),
           Hardwood = pct_count(x = x, r = 4),
           `Conifer-Hardwood` = pct_count(x = x, r = 5),
           `Open Water` = pct_count(x = x, r = 6),
           Quarries = pct_count(x = x, r = 7),
           `Developed-Low Intensity` = pct_count(x = x, r = 8),
           `Developed-Medium Intensity` = pct_count(x = x, r = 9),
           `Developed-High Intensity` = pct_count(x = x, r = 10),
           `Developed-Roads` = pct_count(x = x, r = 11),
           `Developed` = pct_count(x = x, r = 12),
           Agricultural = pct_count(x = x, r = 13),
           Riparian = pct_count(x = x, r = 14),
           `Sparsely Vegetated` = pct_count(x = x, r = 15),
           `Exotic Tree-Shrub` = pct_count(x = x, r = 16),
           `Ruderal Wet Meadow and Marsh` = pct_count(x = x, r = 17),
           `Exotic Herbaceous` = pct_count(x = x, r = 18))
  }) %>%
  exec(bind_rows, .) ->
...
points_mig_pa_results %>%
  pivot_longer(cols = `Grassland`:`Exotic Herbaceous`, names_to = "landcov") ->

...
```{r}
used <- st_read(dsn = here("methods", "Penn_migration_model","dispersing_points_pa_6_2_2021.shp")) %>% 
...
env <- subset(env, c("lsm_1km_dev", "lsm_10km_ed",
"lsm_10km_agri", "lsm_10km_dev", "elev_30m",
"slope_30m", "soil_drainage_30m", "twi_30m", "forest_30m"))
...
Calculating the boyce index (Hirzel et al. (2006)) using cross validation
```{r}
boyce_values <- map2_dbl(1:5,list(model_1, model_2, model_3, model_4, model_5), function(x, k){
...
for(i in 1:nrow(fishnet)){
  ...
  crop(x = env, y = temp_extent, 
       filename = here("methods", "Penn_migration_model", "predictor_stack", paste0("predictor_stack_30m_",i,".grd")), overwrite = TRUE) 
}
```

...
#tss(model, test = TRUE) #0.5601317 #tss is not appropriate for presence-only

# calculating the boyce index (Hirzel et al. (2006))

rf_predictions <- tibble(pres_abs = swd@pa,
                         value = predict(model, swd))

predicted_rast <- terra::rast(here("methods", "Penn_migration_model", "output_layer","rf_mig_5_15_24.tif")) #this is a bit circular, but bear with me

predicted_used <- rf_predictions %>% 
  filter(pres_abs == 1) %>% 
  pull(value)

predicted_background <- terra::values(predicted_rast)

predicted_background <- predicted_background[!is.na(predicted_background)]

boyce_results <- ecospat.boyce(predicted_background, predicted_used, nclass = 10)

boyce_results$cor #0.205

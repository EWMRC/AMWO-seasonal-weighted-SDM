...
```{r}
predictor_stack_full <- raster::stackOpen(here("methods", "Penn_migration_model", "predictor_stack_full_mig.stk"))
...
env <- subset(env, c("lsm_10km_agri", "lsm_10km_dev", "elev_30m","slope_30m", "soil_drainage_30m"))
...
auc(model, test = TRUE) #0.6896137
tss(model, test = TRUE) #0.4027401
...
for(i in 1:nrow(fishnet)){
  temp_extent <- fishnet[i,] %>% as_Spatial() %>% raster::extent()
  raster::crop(x = env, y = temp_extent, 
       filename = here("methods", "Penn_residential_model", "predictor_stack", paste0("predictor_stack_30m_",i,".grd")), overwrite = TRUE) 
}
```
...
# 
# predictor_stack_iter <- brick('/home/aublab001/Penn_residential_model/predictor_stack_3/predictor_stack_3_30m_", index,".grd')
# predictor_stack_iter <- stack(predictor_stack_iter)
# 
# print('predicting variables')
# 
# f <- list(ecoregions = 1:11, sclass = c(1,2,3,4,5,6,7,111,120,132,180))
# 
...
# print('averaging')
# 
# calc(pred_stack, fun = mean, filename = '/home/aublab001/Penn_residential_model/predict_output/predict_", index,".tif')
# 
# print('done')
#              "))
#   sink()

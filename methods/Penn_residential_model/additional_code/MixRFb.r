...
#party::cforest_control(ntree = 2000) #
...
MixRFb <- function(Y, x, random, data, initialRandomEffects=0,
  ...
  while(ContinueCondition0) {
    ...
    while(ContinueCondition) {
      ...
      rf = party::cforest(f1, data=data, weights = weights, control = party::cforest_unbiased(mtry = mtry))
      
      # y - X*beta (out-of-bag prediction)
      pred = predict(rf, OOB = FALSE) #OOB = TRUE
      resi = y - pred
      
      ## Estimate New Random Effects and Errors using lmer
      lmefit <- lme4::lmer(f0, data=data, weights=weights)
  ...
  result <- list(forest=rf, MixedModel=lmefit, RandomEffects=lme4::ranef(lmefit),
...
predict.MixRF <- function(object, newdata, EstimateRE=TRUE){
  ...
  forestPrediction <- predict(object$forest,newdata=newdata,OOB=T)# T

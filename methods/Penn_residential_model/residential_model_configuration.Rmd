...
auc(model, test = TRUE) #0.7171841
tss(model, test = TRUE) #0.4525138
```
...
```
Keeping: 
• Continuous: "lsm_1km_dev", "lsm_10km_ed", "lsm_10km_agri",
"lsm_10km_dev", "elev_30m", "slope_30m", "soil_drainage_30m", and
"twi_30m"
• Categorical: "forest_30m"

# Tune hyperparameters
```{r}
env_hp <- terra::rast(predictor_stack_full)

env_hp <- subset(env_hp, c("lsm_1km_dev", "lsm_10km_ed", "lsm_10km_agri", "lsm_10km_dev", "elev_30m", "slope_30m", "soil_drainage_30m", "twi_30m", "forest_30m"))
...
env_vi <- subset(env_hp, c("lsm_1km_dev", "lsm_10km_ed", "lsm_10km_agri", "lsm_10km_dev", "elev_30m", "slope_30m", "soil_drainage_30m", "twi_30m", "forest_30m"))
...
results_vi
auc(results_vi, test = TRUE) #0.6962879
tss(results_vi, test = TRUE) #0.4102313
```
Final variables:
• Continuous: "lsm_10km_agri", "lsm_10km_dev", "elev_30m",
"slope_30m", and "soil_drainage_30m"
• Categorical: NA
...
env_hp2 <- subset(env_vi, c("lsm_10km_agri", "lsm_10km_dev", "elev_30m","slope_30m", "soil_drainage_30m"))
...
tuned_hypers_2@models[[1]] %>% auc(test = TRUE) #0.8232906 now 0.695207
tuned_hypers_2@models[[1]] %>% tss(test = TRUE) #0.6181498 now 0.4262695
```

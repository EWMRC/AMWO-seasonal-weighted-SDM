...
auc(model, test = TRUE) #0.726965 0.7171841
tss(model, test = TRUE) #0.4666164 0.4525138
```
...
env_vi <- subset(env_vi, c("lsm_1km_dev", "lsm_10km_ed", "lsm_10km_agri", "lsm_10km_dev", "elev_30m", "slope_30m", "soil_drainage_30m", "twi_30m", "forest_30m"))
...
results_vi
auc(results_vi, test = TRUE) #0.7848102 0.8041017
tss(results_vi, test = TRUE) #0.5803167 0.5792609
```
Final variables:
• Continuous: "lsm_10km_ed", "lsm_10km_agri", "lsm_10km_dev", "elev_30m", and
"slope_30m"
• Categorical: "survey"
...
env_hp2 <- subset(env_vi, c("lsm_10km_ed", "lsm_10km_agri", "lsm_10km_dev", "elev_30m", "slope_30m"))
...
tuned_hypers_2@models[[1]] %>% auc(test = TRUE) #0.7970944 now 0.8232906 (note: the old model was 0.83 AUC)
tuned_hypers_2@models[[1]] %>% tss(test = TRUE) #0.5858723 now 0.6181498
```

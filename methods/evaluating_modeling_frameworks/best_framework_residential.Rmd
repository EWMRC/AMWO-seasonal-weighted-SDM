...
RF test
```{r}
rf_full <- train(method = "RF", data = swd_full, swd_full_folds,
               mtry = 5,
               ntree = 500,
               nodesize = 1)

auc(rf_full, test = TRUE) #0.717668
...
tss(rf_full, test = TRUE) #0.4512318
```
...
auc(brt_unbiased, test = TRUE) #0.6323236
#The TSS scores are measured from 1 to + 1, with 1 signifying perfect agreement and 0.60 to 0.90 suggesting good to excellent model performance (Allouche et al.
tss(brt_unbiased, test = TRUE) #0.3236551
```
